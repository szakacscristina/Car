...
public class ConsoleD {
        ...
        public void interpretRequirements(String requirement) {
            ...
            if (option[0].toLowerCase().contains("transaction")) {
                if (option[0].equalsIgnoreCase("addtransaction")) {
                    ...
                    handleAddUpdateTransaction(option[1],option[2], option[3], option[4], option[5], Double.parseDouble(option[6]), Double.parseDouble(option[7]), Double.parseDouble(option[8]));
                }
        ...
        private void handleAddUpdateClient(String id, String lastName, String firstName, String CNP, String dateOfBirth,String dateOfRegistration) {
            try {
                clientService.addOrUpdate(c.getId(), id, lastName, firstName, CNP, dateOfBirth, dateOfRegistration);
        ...
        private void handleAddUpdateTransaction(String id,String idCar, String idClientCard, String date, String time, double pieceTotal, double workmanship, double discount) {
            try {
                Transaction transaction = transactionService.addOrUpdate(id, idCar, idClientCard, pieceTotal, date, time, workmanship, discount);
